...
export default function AdminOrders() {
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>Loading...</div>
  ...
  return (
    <div style={{ background: '#0a0a0a', minHeight: '100vh' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2rem', background: '#111', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
        ...
        <button onClick={handleLogout} style={{ fontSize: 11, color: '#888', border: '0.5px solid rgba(255,255,255,0.08)', padding: '6px 14px', borderRadius: 2, background: 'transparent', cursor: 'pointer' }}>Sign out</button>
      </nav>
        ...
        {[
          ...
          <div key={s.label} style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 4, padding: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontSize: 9, color: '#aaa', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 6 }}>{s.label}</div>
      ...
      <div style={{ padding: '1.5rem 2rem', display: 'grid', gridTemplateColumns: selected ? '1fr 420px' : '1fr', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ background: '#111', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}>
            {filtered.length === 0 ? (
              <div style={{ padding: '3rem', textAlign: 'center', color: '#aaa', fontSize: 13 }}>No orders found</div>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#0d0d0d' }}>
                    {['Order','Client','Business','Items','Units','Amount','Payment','Status','Date',''].map(h => (
                      <th key={h} style={{ fontSize: 8, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#999', padding: '10px 12px', textAlign: 'left', fontWeight: 400, borderBottom: '0.5px solid rgba(255,255,255,0.04)', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={order.id} onClick={() => setSelected(isSelected ? null : order)} style={{ borderTop: '0.5px solid rgba(255,255,255,0.03)', background: isSelected ? 'rgba(45,125,210,0.06)' : 'transparent', cursor: 'pointer', transition: 'background 0.1s' }}>
                        ...
                        <td style={{ padding: '12px 12px' }}>
                          ...
                          <div style={{ fontSize: 9, color: '#aaa', maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{emailMatch}</div>
                        </td>
                        <td style={{ padding: '12px 12px' }}>
                          ...
                          <div style={{ fontSize: 9, color: '#666' }}>{client?.business_type || ''}</div>
                        </td>
                        <td style={{ padding: '12px 12px', fontSize: 12, color: '#aaa', textAlign: 'center' }}>{order.order_items?.length || 0}</td>
                        ...
                        <td style={{ padding: '12px 12px', fontSize: 10, color: '#aaa', whiteSpace: 'nowrap' }}>{fmtDate(order.submitted_at)}</td>
                        <td style={{ padding: '12px 12px' }}>
            ...
            <div style={{ background: '#111', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}>
              ...
              <div style={{ background: '#0d0d0d', padding: '1.25rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: 10, color: '#888' }}>{fmtDate(selected.submitted_at)} at {fmtTime(selected.submitted_at)}</div>
                </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{ padding: '1rem 1.5rem', borderBottom: '0.5px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)' }}>
                    <div style={{ fontSize: 9, color: '#aaa', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10 }}>Client information</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      {[
                        ...
                        <div key={label}>
                          <div style={{ fontSize: 8, color: '#aaa', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 2 }}>{label}</div>
                    ...
                    {payment && (
                      <div style={{ marginTop: 10, padding: '8px 12px', background: 'rgba(45,125,210,0.08)', border: '0.5px solid rgba(45,125,210,0.2)', borderRadius: 3 }}>
                        <div style={{ fontSize: 9, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 3 }}>Payment method requested</div>
              ...
              <div style={{ padding: '1rem 1.5rem', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
                <div style={{ fontSize: 9, color: '#aaa', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10 }}>Items ordered</div>
                {selected.order_items?.map((item, i) => (
                  <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < selected.order_items.length - 1 ? '0.5px solid rgba(255,255,255,0.04)' : 'none' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 9, color: '#aaa', fontFamily: 'monospace' }}>{item.product_sku} · {item.quantity} units × ${item.unit_price?.toLocaleString()}</div>
                    </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, paddingTop: 10, borderTop: '0.5px solid rgba(255,255,255,0.06)' }}>
                  <div>
                    <div style={{ fontSize: 9, color: '#aaa', marginBottom: 2 }}>{selected.order_items?.length || 0} products · {selected.order_items?.reduce((s, i) => s + i.quantity, 0) || 0} total units</div>
                  </div>
              ...
              <div style={{ padding: '1rem 1.5rem' }}>
                <div style={{ fontSize: 9, color: '#aaa', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 10 }}>Update status</div>
                {nextStatus[selected.status] ? (
                  ...
                  <div style={{ fontSize: 12, color: '#aaa', textAlign: 'center', padding: '0.75rem', background: 'rgba(255,255,255,0.02)', borderRadius: 3, marginBottom: 8 }}>
