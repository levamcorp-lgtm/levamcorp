  ...
  return (
    <div style={{ background: '#0a0a0a', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 420px', height: 'calc(100vh - 57px)' }}>
        ...
        <div style={{ overflowY: 'auto', padding: '1.5rem 2rem', borderRight: '0.5px solid rgba(255,255,255,0.06)' }}>
          <div style={{ marginBottom: '1.25rem' }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#fff', marginBottom: 4 }}>Select products to broadcast</div>
            <div style={{ fontSize: 12, color: '#555' }}>{selected.length} selected · {products.length} available</div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: '1rem', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search products..."
                style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '0.5px solid rgba(255,255,255,0.1)', color: '#ccc', fontSize: 12, padding: '8px 12px 8px 28px', borderRadius: 20, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
              <span style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', fontSize: 12, color: '#555' }}>🔍</span>
            </div>
            {cats.map(c => (
              <button key={c} onClick={() => setFilter(c)}
                style={{ fontSize: 11, padding: '6px 12px', borderRadius: 20, border: `0.5px solid ${filter===c?'#25D366':'rgba(255,255,255,0.08)'}`, background: filter===c?'rgba(37,211,102,0.1)':'transparent', color: filter===c?'#25D366':'#555', cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>
                {c}
              </button>
            ))}
          </div>
            ...
            {visible.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => toggle(p.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: isSel ? 'rgba(37,211,102,0.06)' : '#111', border: `1px solid ${isSel ? 'rgba(37,211,102,0.4)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 8, cursor: 'pointer', transition: 'all 0.15s' }}>
                  {/* Checkbox */}
                  ...
                  {/* Image */}
                  {p.image_url
                    ? <img src={p.image_url} alt={p.name} style={{ width: 44, height: 44, objectFit: 'contain', borderRadius: 6, background: '#1a1a1a', flexShrink: 0 }} />
                    : <div style={{ width: 44, height: 44, borderRadius: 6, background: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>📦</div>
                  }
                  {/* Info */}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#555' }}>{p.brand} · MOQ {p.moq || 1} · {p.stock} units</div>
                  </div>
                  {/* Price */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto' }}>
          ...
          {/* Template selector */}
          <div style={{ padding: '1.25rem 1.5rem', borderBottom: '0.5px solid rgba(255,255,255,0.06)', background: '#111' }}>
            <div style={{ fontSize: 11, color: '#888', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, fontWeight: 700 }}>Message format</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
              {[['full','📋 Full','All details'],['compact','⚡ Compact','Quick view'],['promo','🔥 Promo','Sales pitch']].map(([k,l,sub])=>(
                <button key={k} onClick={() => setTemplate(k)}
                  style={{ padding: '8px', background: template===k?'rgba(37,211,102,0.1)':'rgba(255,255,255,0.03)', border: `1px solid ${template===k?'rgba(37,211,102,0.4)':'rgba(255,255,255,0.08)'}`, borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'center' }}>
          ...
          <div style={{ padding: '1rem 1.5rem', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Header</div>
              <textarea value={header} onChange={e => setHeader(e.target.value)} rows={2}
                style={{ width: '100%', background: '#1a1a1a', border: '0.5px solid rgba(255,255,255,0.08)', color: '#ccc', fontSize: 11, padding: '8px', borderRadius: 4, outline: 'none', fontFamily: 'inherit', resize: 'none', boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Note (optional)</div>
              <input value={note} onChange={e => setNote(e.target.value)} placeholder="e.g. Valid until Friday, limited stock..."
                style={{ width: '100%', background: '#1a1a1a', border: '0.5px solid rgba(255,255,255,0.08)', color: '#ccc', fontSize: 11, padding: '8px', borderRadius: 4, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Footer</div>
          ...
          <div style={{ flex: 1, padding: '1rem 1.5rem', overflowY: 'auto' }}>
            <div style={{ fontSize: 11, color: '#888', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8, fontWeight: 700 }}>
            ...
            {selected.length === 0
              ...
              : (
                <div style={{ background: '#0d1f0d', border: '0.5px solid rgba(37,211,102,0.15)', borderRadius: 10, padding: '1rem', fontFamily: 'monospace', fontSize: 12, color: '#ccc', lineHeight: 1.8, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                  {message}
                </div>
              )
            }
          ...
          {/* Send buttons */}
          {selected.length > 0 && (
            <div style={{ padding: '1rem 1.5rem', borderTop: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 8, background: '#0d0d0d' }}>
              <button onClick={openWhatsApp}
                style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg, #25D366, #128C7E)', color: '#fff', fontSize: 14, fontWeight: 800, border: 'none', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 4px 20px rgba(37,211,102,0.3)', letterSpacing: '0.04em' }}>
              ...
              <button onClick={copy}
                style={{ width: '100%', padding: '11px', background: copied ? 'rgba(37,211,102,0.1)' : 'rgba(255,255,255,0.06)', color: copied ? '#25D366' : '#aaa', fontSize: 13, fontWeight: 600, border: `0.5px solid ${copied ? 'rgba(37,211,102,0.3)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 8, cursor: 'pointer' }}>
