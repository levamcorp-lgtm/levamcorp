    ...
    <div style={{ background: '#f4f5f7', minHeight: '100vh' }}>
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 2rem', background: '#fff', borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 500, letterSpacing: '0.15em', color: '#333', textTransform: 'uppercase' }}>Levam Admin</div>
                ...
                <thead>
                  <tr style={{ background: '#f8f9fa' }}>
                    {['Order', 'Client email', 'Method', 'Amount', 'Status', 'Date', ''].map(h => (
                      <th key={h} style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#444', padding: '10px 1.25rem', textAlign: 'left', fontWeight: 400, borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {payments.map(payment => {
                    ...
                    return (
                      <tr key={payment.id} style={{ borderTop: '0.5px solid rgba(0,0,0,0.04)', background: selected?.id === payment.id ? 'rgba(45,125,210,0.05)' : 'transparent', cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '12px 1.25rem', fontSize: 12, fontWeight: 600, color: '#333' }}>#{payment.orders?.order_number || '—'}</td>
                        <td style={{ padding: '12px 1.25rem', fontSize: 11, color: '#333' }}>{payment.client_email || clientEmail}</td>
                        <td style={{ padding: '12px 1.25rem' }}>
                          <span style={{ fontSize: 11, color: '#333' }}>{methodIcons[payment.payment_method]} {methodLabels[payment.payment_method]}</span>
                        </td>
                        <td style={{ padding: '12px 1.25rem', fontSize: 13, fontWeight: 600, color: '#111' }}>${payment.amount?.toLocaleString()}</td>
                        <td style={{ padding: '12px 1.25rem' }}>
            ...
            <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.06)', borderRadius: 4, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '1.25rem 1.5rem', borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <label style={{ fontSize: 10, color: '#999', letterSpacing: '0.15em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Client email *</label>
                  <input type="email" value={clientEmailInput} onChange={e => setClientEmailInput(e.target.value)}
                    placeholder="client@business.com"
                    style={{ width: '100%', background: '#f0f1f3', border: '0.5px solid rgba(0,0,0,0.08)', color: '#444', fontSize: 12, padding: '10px 12px', borderRadius: 2, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                </div>

                {selected.payment_method === 'melio' ? (
                  <div style={{ marginBottom: '1rem' }}>
                    <label style={{ fontSize: 10, color: '#999', letterSpacing: '0.15em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Melio payment link *</label>
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    <label style={{ fontSize: 10, color: '#999', letterSpacing: '0.15em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>
                    ...
                    <textarea value={bankDetails} onChange={e => setBankDetails(e.target.value)}
                      ...
                      style={{ width: '100%', background: '#f0f1f3', border: '0.5px solid rgba(0,0,0,0.08)', color: '#444', fontSize: 12, padding: '10px 12px', borderRadius: 2, outline: 'none', fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' }} />
                  </div>
                )}

                <div style={{ marginBottom: '1.25rem' }}>
                  <label style={{ fontSize: 10, color: '#999', letterSpacing: '0.15em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Additional notes (optional)</label>
                  <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
                    placeholder="Any additional instructions for the client..."
                    style={{ width: '100%', background: '#f0f1f3', border: '0.5px solid rgba(0,0,0,0.08)', color: '#444', fontSize: 12, padding: '10px 12px', borderRadius: 2, outline: 'none', fontFamily: 'inherit', resize: 'none', boxSizing: 'border-box' }} />
                </div>
                ...
                {sent ? (
                  ...
                  <button onClick={sendPaymentLink} disabled={sending} style={{ width: '100%', padding: 11, background: sending ? '#333' : '#2d7dd2', color: '#111', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', border: 'none', cursor: sending ? 'not-allowed' : 'pointer', borderRadius: 2 }}>
