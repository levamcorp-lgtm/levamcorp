...
export default function AdminClients() {
  ...
  return (
    <div style={{ background: '#0a0a0a', minHeight: '100vh' }}>
      ...
      {/* STATS */}
      <div style={{ padding: '1rem 2rem', background: '#111', borderBottom: '0.5px solid rgba(255,255,255,0.06)', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {[
            ...
            {filtered.map(client => {
              ...
              return (
                <div key={client.id} onClick={() => { setSelected(isSelected ? null : client); setCredEmail(client.email); setSent(false); setTab('info') }}
                  style={{ background: '#111', border: `1px solid ${isSelected ? '#2d7dd2' : 'rgba(255,255,255,0.06)'}`, borderLeft: `4px solid ${isSelected ? '#2d7dd2' : 'rgba(45,125,210,0.3)'}`, borderRadius: 6, padding: '1.25rem 1.5rem', cursor: 'pointer' }}>
                  ...
                  {lastOrder && (
                    <div style={{ marginTop: 8, paddingTop: 6, borderTop: '0.5px solid rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {tab === 'documents' && (
                  <div style={{ padding: '1.25rem 1.5rem' }}>
                    ...
                    {[
                      ...
                      <div key={doc.label} style={{ background: doc.url ? 'rgba(45,125,210,0.06)' : 'rgba(255,255,255,0.02)', border: `1px solid ${doc.url ? 'rgba(45,125,210,0.2)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 6, padding: '1.25rem', marginBottom: '1rem' }}>
                        ...
                        <div style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.04)', borderRadius: 4, marginBottom: '0.75rem' }}>
                          <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 3 }}>{doc.numLabel}</div>
                          <div style={{ fontSize: 14, fontWeight: 700, color: doc.num ? '#fff' : '#444' }}>{doc.num || 'Not provided'}</div>
                        </div>
                        {doc.url ? (
                          <button onClick={() => openDoc(doc.url)} style={{ width: '100%', padding: '11px', background: '#2d7dd2', color: '#fff', fontSize: 12, fontWeight: 700, border: 'none', cursor: 'pointer', borderRadius: 4, boxShadow: '0 4px 14px rgba(45,125,210,0.3)' }}>
                            📄 Open / Download PDF
                          </button>
                        ) : (
                      ...
                      <>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: '1rem' }}>
                          {[
                            ...
                            <div key={label} style={{ padding: '8px', background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 4, textAlign: 'center' }}>
                              <div style={{ fontSize: 8, color: '#555', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 3 }}>{label}</div>
                              <div style={{ fontSize: 14, fontWeight: 700, color }}>{val}</div>
                            </div>
                          ))}
                        </div>
                        {getClientOrders(selected).map(order => (
                          <div key={order.id} style={{ padding: '12px', marginBottom: 8, background: 'rgba(255,255,255,0.02)', border: '0.5px solid rgba(255,255,255,0.06)', borderLeft: `3px solid ${statusColor[order.status] || '#555'}`, borderRadius: 4 }}>
                ...
                {tab === 'credentials' && (
                  <div style={{ padding: '1.25rem 1.5rem' }}>
                    <div style={{ fontSize: 12, color: '#888', lineHeight: 1.7, marginBottom: '1.25rem', padding: '10px 14px', background: 'rgba(255,200,0,0.04)', border: '0.5px solid rgba(255,200,0,0.1)', borderRadius: 4 }}>
                      ⚠️ First create the user in <strong style={{ color: '#ccc' }}>Supabase → Authentication → Users → Add user</strong>, then send credentials here.
                    </div>
